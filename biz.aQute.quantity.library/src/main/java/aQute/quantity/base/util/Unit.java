	...
	public String toString() {
		if (unit != null) {
			...
			for (Dimension dimension : dimensions) {
				int exponent = dimension.exponent;
				if (exponent == 0)
					continue;

				
				sb.append(del).append(info.unit());
				if (exponent != 1) {
					if (exponent < 0) {
						sb.append("⁻");
						exponent = -exponent;
					...
					if (exponent <= 9) {
						sb.append(EXPONENT.charAt(exponent));
					} else {
						String exps = Integer.toString(exponent);
						for (int i = 0; i < exps.length(); i++) {
							sb.append(EXPONENT.charAt(exps.charAt(i) - '0'));
						}
					}
				}
				del = " ";
