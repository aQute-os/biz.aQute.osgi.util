...
(function (options) {
  ...
  var DiagramMultiContainer = (funcs.DiagramMultiContainer = function DiagramMultiContainer(
    tagName,
    items,
    attrs,
    text
  ...
  var Diagram = (funcs.Diagram = function Diagram(items) {
    ...
    DiagramMultiContainer.call(this, "svg", items, {
    ...
    for (var i = 0; i < this.items.length; i++) {
      ...
      if (i > 0) {
        this.height += Math.max(
          ...
          item.up + Diagram.VERTICAL_SEPARATION
        );
      }
      if (i < last) {
        this.height += Math.max(
          Diagram.ARC_RADIUS * 2,
          item.down + Diagram.VERTICAL_SEPARATION
        );
      }
    }
    if (Diagram.DEBUG) {
  ...
  Stack.prototype.format = function (x, y, width) {
    ...
    for (var i = 0; i < this.items.length; i++) {
      var item = this.items[i];
      var innerWidth =
        this.width - (this.items.length > 1 ? Diagram.ARC_RADIUS * 2 : 0);
      ...
      if (i !== this.items.length - 1) {
        Path(x, y)
          ...
          .down(
            Math.max(
              0,
              item.down + Diagram.VERTICAL_SEPARATION - Diagram.ARC_RADIUS * 2
            )
          )
          .arc("es")
          .left(innerWidth)
        ...
        y +=
          ...
          Math.max(
            this.items[i + 1].up + Diagram.VERTICAL_SEPARATION,
            Diagram.ARC_RADIUS * 2
          );
  ...
  var OptionalSequence = (funcs.OptionalSequence = function OptionalSequence(
    items
  ) {
    if (!(this instanceof OptionalSequence))
      return new OptionalSequence([].slice.call(arguments));
    DiagramMultiContainer.call(this, "g", items);
    if (items.length === 0) {
    ...
    for (var i = 0; i < this.items.length; i++) {
      ...
      this.up = Math.max(
        this.up,
        Math.max(arc * 2, item.up + Diagram.VERTICAL_SEPARATION) - heightSoFar
      );
      ...
      if (i > 0) {
        this.down =
          Math.max(
            ...
            heightSoFar +
              Math.max(arc * 2, item.down + Diagram.VERTICAL_SEPARATION)
          ) - this.height;
      }
    ...
    for (var i = 0; i < this.items.length; i++) {
      ...
      if (i == 0) {
        ...
        Path(x, y)
          ...
          .down(
            item.height +
              Math.max(item.down + Diagram.VERTICAL_SEPARATION, arc * 2) -
              arc * 2
          )
          .arc("ws")
          .right(itemWidth - arc)
  ...
  var AlternatingSequence = (funcs.AlternatingSequence = function AlternatingSequence(
    items
  ) {
    if (!(this instanceof AlternatingSequence))
      return new AlternatingSequence([].slice.call(arguments));
    DiagramMultiContainer.call(this, "g", items);
    if (items.length === 1) {
      return new Sequence(items);
    }
  ...
  AlternatingSequence.prototype.format = function (x, y, width) {
    ...
    Path(x + this.width - 2 * arc, y - firstOut)
      .arc("ne")
      .down(firstOut - 2 * arc)
      .arc("ws")
      .addTo(this);
    ...
    Path(x + this.width - 2 * arc, y + secondOut)
      .arc("se")
      .up(secondOut - 2 * arc)
      .arc("wn")
      .addTo(this);
  ...
  Choice.prototype.format = function (x, y, width) {
    ...
    for (var i = this.normal - 1; i >= 0; i--) {
      ...
      Path(
        ...
        .arc("ne")
        .down(
          distanceFromY - item.height + this.height - Diagram.ARC_RADIUS * 2
        )
        .arc("ws")
        .addTo(this);
  ...
  var HorizontalChoice = (funcs.HorizontalChoice = function HorizontalChoice(
    items
  ) {
    if (!(this instanceof HorizontalChoice))
      return new HorizontalChoice([].slice.call(arguments));
    if (items.length === 0) {
    ...
    this._lowerTrack = Math.max(
      ...
      max(
        ...
        (x) =>
          x.height +
          Math.max(x.down + Diagram.VERTICAL_SEPARATION, Diagram.ARC_RADIUS * 2)
      ),
    ...
    if (first.height < this._lowerTrack) {
      ...
      this._lowerTrack = Math.max(
        this._lowerTrack,
        first.height + Diagram.ARC_RADIUS * 2
      );
    }
  ...
  HorizontalChoice.prototype.format = function (x, y, width) {
    ...
    // Items
    for (var [i, item] of enumerate(this.items)) {
      // input track
      if (i === 0) {
        new Path(x, y).h(Diagram.ARC_RADIUS).addTo(this);
        x += Diagram.ARC_RADIUS;
      } else {
  ...
  var MultipleChoice = (funcs.MultipleChoice = function MultipleChoice(
    normal,
    type,
    items
  ) {
    if (!(this instanceof MultipleChoice))
      return new MultipleChoice(normal, type, [].slice.call(arguments, 2));
    DiagramMultiContainer.call(this, "g", items);
    if (typeof normal !== "number" || normal !== Math.floor(normal)) {
      throw new TypeError(
    ...
    this.width =
      30 + Diagram.ARC_RADIUS + this.innerWidth + Diagram.ARC_RADIUS + 20;
    this.up = this.items[0].up;
    this.down = this.items[this.items.length - 1].down;
    this.height = this.items[normal].height;
    for (var i = 0; i < this.items.length; i++) {
      ...
      if (i == normal - 1 || i == normal + 1)
        var minimum = 10 + Diagram.ARC_RADIUS;
  ...
  MultipleChoice.prototype.format = function (x, y, width) {
    ...
    for (var i = this.normal - 1; i >= 0; i--) {
      ...
      if (i == this.normal - 1) {
        var distanceFromY = Math.max(
          10 + Diagram.ARC_RADIUS,
          normal.up + Diagram.VERTICAL_SEPARATION + item.down + item.height
        );
      }
      Path(x + 30, y)
        ...
        .addTo(this);
      item
        .format(x + 30 + Diagram.ARC_RADIUS, y - distanceFromY, this.innerWidth)
        .addTo(this);
      Path(
        x + 30 + Diagram.ARC_RADIUS + this.innerWidth,
        y - distanceFromY + item.height
      )
        .arc("ne")
        .down(
          distanceFromY - item.height + this.height - Diagram.ARC_RADIUS - 10
        )
        .addTo(this);
      if (i != 0) {
        distanceFromY += Math.max(
    ...
    Path(x + 30 + Diagram.ARC_RADIUS + this.innerWidth, y + this.height)
      .right(Diagram.ARC_RADIUS)
      .addTo(this);
    ...
    for (var i = this.normal + 1; i < this.items.length; i++) {
      ...
      if (i == this.normal + 1) {
        var distanceFromY = Math.max(
          10 + Diagram.ARC_RADIUS,
          normal.height + normal.down + Diagram.VERTICAL_SEPARATION + item.up
        );
      }
      Path(x + 30, y)
  ...
  var Terminal = (funcs.Terminal = function Terminal(
    text,
    { href, title } = {}
  ) {
  ...
  var NonTerminal = (funcs.NonTerminal = function NonTerminal(
    text,
    { href, title } = {}
  ) {
    if (!(this instanceof NonTerminal))
      return new NonTerminal(text, { href, title });
  ...
  var Block = (funcs.Block = function Block({
    width = 50,
    up = 15,
    height = 25,
    down = 15,
    ...
    if (!(this instanceof Block))
      return new Block({ width, up, height, down, needsSpace });
    FakeSVG.call(this, "g");
    this.width = width;
    this.height = height;
    this.up = up;
