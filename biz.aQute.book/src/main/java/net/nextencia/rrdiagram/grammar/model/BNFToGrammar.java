...
public class BNFToGrammar {
	...
	private static class Chunk {
		...
		private void prune() {
				...
				switch (chunk.getType()) {
					case REPETITION_TOKEN : {
						if ("*".equals(chunk.text)) {
							...
							if (multiplier != null) {
								// The current one is removed, so next one is at
								// index i.
			...
			if (hasAlternation) {
				List<List<Chunk>> alternationSequenceList = new ArrayList<>();
		...
		private Expression getExpression() {
			switch (type) {
				...
				case CHOICE : {
					...
					for (Chunk chunk : chunkList) {
						...
						if (expression instanceof Repetition) {
							...
							if (repetition.getMinRepetitionCount() == 0) {
								if (repetition.getMaxRepetitionCount() == null
									|| repetition.getMaxRepetitionCount() != 1) {
									expression = new Repetition(repetition.getExpression(), 1,
										repetition.getMaxRepetitionCount());
								} else {
					...
					if (hasLine
						&& (expressionList.isEmpty() || !isNoop(expressionList.get(expressionList.size() - 1)))) {
				...
				case REPETITION : {
					if (chunkList.size() == 1) {
						return new Repetition(chunkList.get(0)
							.getExpression(), minCount, maxCount);
					}
					List<Expression> expressionList = new ArrayList<>();
					for (Chunk chunk : chunkList) {
						expressionList.add(chunk.getExpression());
				...
				if (isInSpecialGroup) {
					...
					switch (c) {
						...
						case '\t' : {
							...
							sb.delete(0, sb.length());
							// parentChunk.addChunk(new
							// Chunk(ChunkType.CONCATENATION));
							break;
						}
						case '|' : {
							String content = sb.toString()
