...
public class Repetition extends Expression {
	...
	protected RRElement toRRElement(GrammarToRRDiagram grammarToRRDiagram) {
		...
		if (minRepetitionCount == 0) {
			if (maxRepetitionCount == null || maxRepetitionCount > 1) {
				return new RRChoice(
					new RRLoop(rrElement, null, 0, (maxRepetitionCount == null ? null : maxRepetitionCount - 1)),
					new RRLine());
			}
			return new RRChoice(rrElement, new RRLine());
		}
		return new RRLoop(rrElement, null, minRepetitionCount - 1,
	...
	public int hashCode() {
		...
		result = prime * result + ((maxRepetitionCount == null) ? 0 : maxRepetitionCount.hashCode());
		result = prime * result + minRepetitionCount;
		return result;
	}
	...
	public boolean equals(Object obj) {
		...
		if (expression == null) {
			...
			return false;
		if (maxRepetitionCount == null) {
			if (other.maxRepetitionCount != null)
				return false;
		} else if (!maxRepetitionCount.equals(other.maxRepetitionCount))
			return false;
		if (minRepetitionCount != other.minRepetitionCount)
			return false;
		return true;
	}
