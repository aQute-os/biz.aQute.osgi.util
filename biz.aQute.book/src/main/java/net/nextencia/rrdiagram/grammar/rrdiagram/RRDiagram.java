...
public class RRDiagram {
	...
	public static class SvgContent {
		private List<SvgConnector> connectorList = new ArrayList<>();
		...
		private String getConnectorElement(RRDiagramToSVG rrDiagramToSVG) {
			...
			return "<path class='path' d=\"" + path0.getPath() + "\"/>" + SVG_ELEMENTS_SEPARATOR;
		}

		private StringBuilder elementsSB = new StringBuilder();

		public void addElement(String element) {
			elementsSB.append(element)
	...
	String toSVG(RRDiagramToSVG rrDiagramToSVG) {
		...
		if (rrElement instanceof RRSequence) {
			List<RRElement> cursorElementList = new ArrayList<>();
			for (RRElement element : ((RRSequence) rrElement).getRRElements()) {
				if (element instanceof RRBreak) {
					if (!cursorElementList.isEmpty()) {
						rrElementList.add(cursorElementList.size() == 1 ? cursorElementList.get(0)
							: new RRSequence(cursorElementList.toArray(new RRElement[0])));
		...
		// Then generate the rest (CSS and SVG container tags) based on that
		// usage.
		StringBuilder sb = new StringBuilder();
		sb.append(
			"<svg version=\"1.1\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" xmlns=\"http://www.w3.org/2000/svg\" width=\"")
			.append(width)
			.append("\" height=\"")
			.append(height)
			.append("\" viewbox=\"0 0 ")
