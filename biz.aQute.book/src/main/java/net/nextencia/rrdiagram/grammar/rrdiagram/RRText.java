...
public class RRText extends RRElement {
	...
	protected void toSVG(RRDiagramToSVG rrDiagramToSVG, int xOffset, int yOffset, SvgContent svgContent) {
		...
		if (link != null) {
			svgContent.addElement("<a xlink:href=\"" + Utils.escapeXML(
				link)/* + "\" xlink:title=\"" + Utils.escapeXML(text) */ + "\">");
		}
		Insets insets;
		Font font;
		String cssClass;
		...
		switch (shape) {
			case RECTANGLE :
				svgContent.addElement("<rect class=\"" + cssClass + "\" x=\"" + xOffset + "\" y=\"" + (yOffset + 4)
					+ "\" width=\"" + width + "\" height=\"" + (height - 4) + "\"/>");
				break;
			case ROUNDED_RECTANGLE :
				// Connector may be in rounded area if there are huge margins at
				// top, but this is an unrealistic case so we don't add lines to
				// complete the connector.
				int rx = (insets.left + insets.right + insets.top + insets.bottom) / 4;
				svgContent.addElement("<rect class=\"" + cssClass + "\" x=\"" + xOffset + "\" y=\"" + (yOffset + 4)
					+ "\" width=\"" + width + "\" height=\"" + (height - 4) + "\" rx=\"" + rx + "\"/>");
				break;
			case HEXAGON :
				// We don't calculate the exact length of the connector: it goes
				// behind the shape.
				// We should calculate if we want to support transparent shapes.
				int connectorOffset = layoutInfo.getConnectorOffset();
				svgContent.addLineConnector(xOffset, yOffset + connectorOffset, xOffset + insets.left,
					yOffset + connectorOffset);
				svgContent.addElement("<polygon class=\"" + cssClass + "\" points=\"" + xOffset + " "
					+ (yOffset + height / 2) + " " + (xOffset + insets.left) + " " + yOffset + " "
					+ (xOffset + width - insets.right) + " " + yOffset + " " + (xOffset + width) + " "
					+ (yOffset + height / 2) + " " + (xOffset + width - insets.right) + " " + (yOffset + height) + " "
					+ (xOffset + insets.left) + " " + (yOffset + height) + "\"/>");
				svgContent.addLineConnector(xOffset + width, yOffset + connectorOffset, xOffset + width - insets.right,
					yOffset + connectorOffset);
				break;
		}
		FontRenderContext fontRenderContext = new FontRenderContext(null, true, false);
		Rectangle2D stringBounds = font.getStringBounds(text, fontRenderContext);
		int textXOffset = xOffset + width / 2;
		int textYOffset = yOffset + insets.top + 2 + (int) Math.round(stringBounds.getHeight()) - fontYOffset;
		svgContent.addElement("<text class=\"" + cssTextClass + "\" x=\"" + textXOffset + "\" y=\"" + textYOffset
			+ "\">" + Utils.escapeXML(text) + "</text>");
		if (link != null) {
